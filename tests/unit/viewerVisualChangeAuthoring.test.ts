...
describe('visual-change Viewer HTTP surface', () => {
  ...
  it('keeps every POST project-aware, token/Host/Origin/JSON guarded, closed, bounded, and no-store', async () => {
    ...
    expect((await post(ctx, '/api/visual-changes', { scope, extra: true })).status).toBe(400); for (const action of ['activate','check','restore-acceptance','prepare-handoff','review','governance']) expect((await post(ctx, `/api/visual-changes/${encodeURIComponent(ctx.handle)}/${action}`, { extra: true })).status).toBe(400);
